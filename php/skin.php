...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['arquivo']) && $_FILES['arquivo']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($_FILES['arquivo']['size'] > 40960) {
            echo "Erro: O tamanho do arquivo deve ser no máximo 40KB.";
        } else {
            // Verifica o tipo de arquivo (apenas PNG)
            $tipoArquivo = $_FILES['arquivo']['type'];
            if ($tipoArquivo != 'image/png') {
                echo "Erro: O arquivo deve ser do tipo PNG.";
            } else {
                // Define variáveis para o arquivo
                $skin = $_FILES['arquivo'];
                $nomeAleatorio = uniqid('arquivo_') . '_' . time();
